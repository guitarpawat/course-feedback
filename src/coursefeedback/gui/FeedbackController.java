...
/**
 * Controller for Feedback with user interface.
 * 
...
public class FeedbackController extends DBQuery implements Observer {
	...
	/**
	 * The method for sending the data that students evaluate teachers. There are 5
	 * questions and 6 answers that students must to evaluate.
	 * 
	 * @param event
	 *            is an action event.
	 */
	public void sendFeedback(ActionEvent event) {
		...
		super.setPreparedCommand(
				"INSERT INTO feedback (userid,courseid,section,q1,q2,q3,q4,q5,comment) VALUES (?,?,?,?,?,?,?,?,?)");
		super.addBindValue(student.getUserID());
		super.addBindValue(course.getCourseID());
		super.addBindValue(course.getSection());
		for (int n : score) {
			super.addBindValue(n);
		}
